...
void Camera::MouseLook(int mouseX, int mouseY){
    ...
    float xDistance = m_oldMousePosition.x-newMousePosition.x;
    m_viewDirection = glm::rotate(
            glm::normalize(m_viewDirection),
            glm::radians(xDistance),
            glm::normalize(m_upVector));

    // looks up and down
    float yDistance = m_oldMousePosition.y-newMousePosition.y;
    m_viewDirection = glm::rotate(
            glm::normalize(m_viewDirection),
            glm::radians(yDistance),
            glm::normalize(glm::cross(m_viewDirection, m_upVector))); // use right vector
...
void Camera::MoveLeft(float speed){
    glm::vec3 rightVector = glm::normalize(glm::cross(m_upVector, m_viewDirection));
    m_eyePosition = m_eyePosition - (rightVector* speed);
}
